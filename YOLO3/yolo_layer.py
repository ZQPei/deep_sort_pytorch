...
class YoloLayer(nn.Module):
    def __init__(
        self, anchor_mask=[], num_classes=0, anchors=[], num_anchors=1, use_cuda=None
    ):
    ...
    def forward(self, output, target):
        ...
        t2 = time.time()
        nGT, nRecall, nRecall75, coord_mask, conf_mask, cls_mask, tcoord, tconf, tcls = self.build_targets(
            pred_boxes, target.detach(), anchors.detach(), nA, nH, nW
        )
